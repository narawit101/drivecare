    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/50 p-4 ">
      <div className="bg-white w-full max-w-100 h-[600px] overflow-y-auto rounded-xl p-5 shadow-2xl animate-in fade-in zoom-in duration-200 hide-scrollbar">
        ...
        <div className="bg-[#70C5BE]/10 rounded-xl p-3 mb-4 text-center">
          <p className="text-xs text-[#3a8b85] font-bold uppercase mb-1">
            ยอดชำระสุทธิ
          </p>
        ...
        <div className="bg-[#70C5BE]/10 rounded-xl p-3 mb-4 text-center group relative">
          ...
          <div>
            ...
            >
              <p className="text-xs text-[#3a8b85] font-bold uppercase mb-1">
                เลขพร้อมเพย์ 0981848532
              </p>
              <Icon
                icon="solar:copy-bold-duotone"
                className="text-[#70C5BE] w-3 h-3 hover:text-[#3a8b85]"
              />
            </div>
          </div>
        </div>
        <div
          ...
          className={`relative w-full aspect-video border-2 border-dashed border-gray-200 rounded-4xl flex flex-col items-center justify-center transition-all overflow-hidden group ${isLoading ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-gray-50"}`}
        >
          {previewUrl ? (
            <>
              <Image
        ...
        <div className="flex gap-4 mt-8">
          <button
            disabled={isLoading}
            className={`flex-1 py-4 text-sm font-bold text-gray-400 transition-colors ${isLoading ? "cursor-not-allowed opacity-55" : "hover:text-gray-600 cursor-pointer"}`}
            onClick={() => {
              setPreviewUrl(null);
