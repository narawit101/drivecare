...
export default function AdminReportPage() {
  ...
  const [assignModalReport, setAssignModalReport] = useState<ReportRow | null>(
    null,
  );
  ...
  const fetchReports = async () => {
    ...
    try {
      const res = await fetch(
        "/api/reports/admin/fetch-reports-with-reporter",
        {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const filteredReports = reports.filter((r) => {
    ...
    if (selectedDate) {
      ...
      if (bookingDateTH !== selectedDate && reportDate !== selectedDate)
        return false;
    }
    ...
    if (search) {
      ...
      return (
        ...
        FormatDatetime.formatThaiNumericDate(r.booking_date).includes(
          keyword,
        ) ||
      ...
      <main className="flex flex-1 flex-col gap-6 overflow-y-auto px-4 py-4 md:px-8 md:py-6 text-slate-900">
        ...
        {!loading && filteredReports.length === 0 ? (
          <section className="flex flex-wrap items-center gap-4 rounded-xl p-4 shadow-sm justify-center bg-white text-center">
            <p className="text-slate-500 text-center flex items-center justify-center">
              ยังไม่มีรายงาน
            </p>
          </section>
        ) : (
