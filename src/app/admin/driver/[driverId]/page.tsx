...
export default function DriverDetailPage() {
  ...
  const updateStatus = async (verified: string, reason?: string) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/admin-controller/drivers/${driverId}/verify`,
        {
          method: "PUT",
  ...
  if (loading)
    return (
      <div className="max-w-5xl mx-auto p-6 text-center">กำลังโหลด...</div>
    );
  if (!driver)
    return <div className="max-w-5xl mx-auto p-6 text-center">ไม่พบข้อมูล</div>;
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-center gap-2 mb-6 border-b border-gray-200 pb-2">
          ...
          {driver?.verified === "approved" ? (
            ...
            <span className="bg-gray-400 border border-gray-400 text-white text-xs px-2 py-1 rounded-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <ImagePreview label="รูปบัตรประชาชน" value={driver?.citizen_id_img} />
          <ImagePreview
            label="รูปใบขับขี่"
        ...
        <div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <ImagePreview label="รูปรถ" value={driver?.car_img} />
            <ImagePreview label="รูป พ.ร.บ. รถ" value={driver?.act_img} />
          </div>
