...
export async function PATCH(request: NextRequest) {
    ...
    try {
        ...
        const check = await pool.query(
            `SELECT booking_id, user_id, driver_id
       FROM bookings
       WHERE booking_id = $1
       FOR UPDATE`,
            [bookingId]
        ...
        const oldDriverId = check.rows[0]?.driver_id as number | null | undefined;
       ...
       RETURNING booking_id, driver_id, status`,
        ...
        await pusher.trigger(`private-driver-${driverId}`, "booking.assigned", {
            ...
            driver_id: driverId,
            type: "ADMIN_ASSIGN",
            message: "แอดมินได้มอบหมายงานให้คุณ"
        });

        // แจ้งคนขับเก่าที่ถูกเปลี่ยน (ถ้ามี)
        if (oldDriverId && oldDriverId !== driverId) {
            await pusher.trigger(`private-driver-${oldDriverId}`, "booking.reassigned", {
                booking_id: bookingId,
                old_driver_id: oldDriverId,
                new_driver_id: driverId,
                type: "DRIVER_REASSIGNED",
                message: "งานนี้ถูกมอบหมายให้คนขับคนอื่นแล้ว"
            });
        }
