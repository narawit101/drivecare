...
import {
  ...
  InputProps,
  StatCardProps,
} from "@/types/user/health-bookinng";
...
export default function HealthDashboard() {
  ...
  const getBmiStatus = (bmi: number) => {
    ...
    if (bmi < 25)
      return { label: "ท้วม / น้ำหนักเกิน", color: "bg-yellow-400" };
        ...
        <div className="bg-[#f8fcfc] min-h-screen pb-24 font-sans">
          ...
          <div className="bg-white  mt-4 p-6 rounded-2xl shadow-sm border border-gray-50">
            <div className="flex justify-between items-center mb-4">
              ...
              <span
                className={`text-[10px] px-3 py-1 rounded-full font-bold text-white ${status.color}`}
              >
                {status.label}
              </span>
            </div>
          ...
          <div className="mt-4 space-y-4">
            <div className="bg-white p-5 rounded-2xl border border-gray-50">
              <p className="text-sm  font-bold text-[#70C5BE] mb-3 uppercase tracking-wider">
                โรคประจำตัว
              </p>
              <div className="flex flex-wrap gap-2">
                {healthData?.congenital_diseases?.length ? (
                  healthData.congenital_diseases.map((d) => (
                    <span
                      key={d}
                      className="bg-[#f2faf9] text-[#3a8b85] text-xs font-bold px-4 py-1.5 rounded-full border border-[#e0f2f0]"
                    >
                      {d}
                    </span>
                  ))
            ...
            <div className="bg-[#fff8f8] p-5 rounded-2xl border border-[#ffebeb]">
              <p className="text-sm font-bold text-[#ff5a5a] mb-3 uppercase tracking-wider">
                การแพ้ยา / อาหาร
              </p>
              <div className="flex flex-wrap gap-2">
                {healthData?.allergies?.length ? (
                  healthData.allergies.map((a) => (
                    <div
                      key={a}
                      className="bg-white text-[#ff5a5a] text-xs font-bold px-4 py-1.5 rounded-full border border-[#ffebeb] flex items-center gap-1"
                    >
            ...
            <div className="fixed inset-0 z-100 bg-black/60 flex items-center justify-center p-3  backdrop-blur-sm">
              ...
              >
                ...
                <Input
                  ...
                  onChange={(v) =>
                    setFormData({ ...formData, congenital_diseases: v })
                  }
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-50 text-center flex flex-col items-center justify-center">
      ...
      <div className="flex flex-col items-center gap-1">
        <span
          className={`text-xl font-black ${isHighlight ? "text-[#70C5BE]" : "text-gray-800"}`}
        >
          {value}
        </span>
        {unit && value !== "-" && (
...
function Input({
  ...
  return (
    <div className="space-y-1">
      <label
        className={`text-xs  uppercase ml-1 ${isAlert ? "text-red-400" : "text-[#70C5BE]"}`}
      >
        {label}
      </label>
      <input
