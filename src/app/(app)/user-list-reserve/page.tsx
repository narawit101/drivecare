...
export default function DriverJob() {
  ...
  const inProgressBookings = booking.filter((b) =>
    activeStatuses.includes(b.status),
  );
  const completedBookings = booking.filter(
    (b) => !activeStatuses.includes(b.status),
  );

  const filteredCompletedBookings =
    statusFilter === "all"
      ? completedBookings
      ...
      <main className="max-w-5xl mx-auto px-6 py-6 space-y-6">
        <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 w-full md:w-[400px]">
          <button
            ...
            className={`flex-1 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${isInProgress ? "bg-[#70C5BE] text-white shadow-md" : "text-slate-400"}`}
          >
          ...
          <button
            ...
            className={`flex-1 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${!isInProgress ? "bg-[#70C5BE] text-white shadow-md" : "text-slate-400"}`}
          >
            ประวัติการจอง ({completedBookings.length})
          </button>
        </div>
        ...
        <div
          className={`grid gap-4 ${isInProgress ? "grid-cols-1" : "grid-cols-1 md:grid-cols-3"}`}
        >
          {!isInProgress && (
            <div className="col-span-full flex gap-2 mb-2">
              {["all", "success", "cancelled"].map((id) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold ${statusFilter === id ? "bg-[#70C5BE]/10 text-[#3a8b85] ring-1 ring-[#70C5BE]" : "bg-white text-slate-400 border border-slate-100"}`}
                >
                  {id === "all"
                    ? "ทั้งหมด"
                    : id === "success"
          ...
          {(isInProgress ? inProgressBookings : paginatedCompletedBookings)
            .length === 0 &&
            !loading && <EmptyState />}
        </div>
...
function BookingCard({
  ...
  const getStatusConfig = (status: string) => {
    const configs: Record<string, { color: string; text: string }> = {
      ...
      heading_to_hospital: {
        color: "bg-[#70C5BE]/10 text-[#3a8b85]",
        text: "กำลังไปโรงพยาบาล",
      },
      arrived_at_hospital: {
        color: "bg-green-100 text-green-700",
        text: "ถึงโรงพยาบาลแล้ว",
      },
      waiting_for_return: {
        color: "bg-amber-100 text-amber-700",
        text: "รอรับผู้ป่วยกลับ",
      },
      heading_home: {
        color: "bg-[#70C5BE]/10 text-[#3a8b85]",
        text: "กำลังกลับบ้าน",
    ...
    return (
      configs[status] || { color: "bg-gray-100 text-gray-500", text: status }
    );
  };
  ...
  return (
    <div className="bg-white rounded-4xl p-6 shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-[#70C5BE]/10 transition-all duration-300 group flex flex-col justify-between h-full">
          ...
          <div className="flex gap-4 items-center">
            ...
            <div>
              ...
              <p className="font-bold text-gray-800">
                {FormatDatetime.formatThaiDate(booking.booking_date)},{" "}
                {FormatDatetime.formatThaiTime(booking.start_time)}
              </p>
            </div>
          </div>
          <span
            className={`px-3 py-1 rounded-full text-[10px] font-bold ${config.color}`}
          >
            {config.text}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10">
              <Image
                src={
                  booking.driver_profile_image || "/images/noprofile-avatar.jpg"
                }
                alt="Driver"
                fill
      ...
      <button
        ...
        className="mt-6 w-full py-3 rounded-2xl bg-slate-50 text-[#3a8b85] font-bold text-sm hover:bg-[#70C5BE] hover:text-white transition-all flex items-center justify-center gap-2"
      >
        <Icon icon="solar:document-list-bold" className="text-xl" />{" "}
        ดูรายละเอียดงาน
      </button>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-4xl p-20 text-center border border-dashed border-slate-200 w-full col-span-full">
      <Icon
        ...
        className="mx-auto text-6xl text-slate-100 mb-4"
      />
